<template>
  <div class="option-list">
    ...
    <div v-else>
      <van-empty v-if="options.length === 0" description="暂无惩罚选项，请添加" />
    ...
    <van-popup v-model:show="showAddForm" position="bottom" :style="{ height: '80%' }">
      <van-nav-bar
        :title="editingOption ? '编辑惩罚选项' : '新增惩罚选项'"
        left-arrow
...
const fetchOptions = async () => {
  loading.value = true
  try {
    options.value = await scoresApi.getPunishmentOptions()
  } catch (error) {
...
const handleDelete = async (option) => {
  try {
    ...
    await scoresApi.deletePunishmentOption(option.id)
